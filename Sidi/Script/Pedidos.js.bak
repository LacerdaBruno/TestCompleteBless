...
function insereGrade()
{
  ...
  Aliases.SIDI.frmInformarGradeVolume.Panel1.gbItens.Panel6.GRID_ITENS_GRADE.Keys("[Right]")
  for(i = 0; i < Project.Variables.Grade.RowCount; i++){
    Aliases.SIDI.frmInformarGradeVolume.Panel1.gbItens.Panel6.GRID_ITENS_GRADE.Keys(Project.Variables.Grade.quantidade(i) + "[Right]");
  }
...
function fazEntrega(){
  // Faz a entrega
  alteraAba("Entregas");
  Aliases.SIDI.frmPrincipal.MDIClient.TelaPedidos.PageControl.tsEntregas.gbEntregas.pnlEntrega.pnlEntregaTitulo.pnlEntregaTitulo.btnEntrega.ClickButton();
